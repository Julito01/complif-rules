...
const DEFAULT_ORG_ID = process.env.DEFAULT_ORG_ID || 'complif-001';
const DATABASE_URI =
  process.env.DATABASE_URI || 'postgresql://complif:complif@localhost:5432/complif';

// ─────────────────────────────────────────────────────────────────────────────
// HTTP Client (minimal, no external dependencies)
// ─────────────────────────────────────────────────────────────────────────────
    ...
    inputSchema: {
      ...
      properties: {
        ...
        window: {
          type: 'object',
          description:
            'Optional sliding window spec: { duration: number, unit: "hours"|"days"|"minutes" }',
        },
        priority: {
          type: 'number',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: {
          type: 'string',
          description:
            'Optional status filter (e.g., "OPEN", "ACKNOWLEDGED", "RESOLVED", "DISMISSED")',
        },
        severity: {
          type: 'string',
...
// ─────────────────────────────────────────────────────────────────────────────
// Rule Evaluation Helpers (client-side, mirrors domain logic for explanation)
// These do NOT modify any state and are purely for explanation purposes.
// ─────────────────────────────────────────────────────────────────────────────
...
function evaluateRule(rule: RuleDefinition, signatures: Record<string, number>): EvaluationResult {
...
async function handleToolCall(name: string, args: Record<string, unknown>): Promise<string> {
  const orgId = (args.organizationId as string) || DEFAULT_ORG_ID;
  ...
  switch (name) {
    ...
    case 'get_signature_rules': {
      const schemaId = args.schemaId as string;
      const facultyId = args.facultyId as string | undefined;
    ...
    case 'simulate_signature_evaluation': {
      const ruleId = args.ruleId as string;
      const signatures = args.signatures as Record<string, number>;

      // Fetch the rule definition first
      const ruleResponse = await httpGet<{
        success: boolean;
        data: { ruleDefinition: RuleDefinition };
      }>(`/signature-rules/${ruleId}`, orgId);
    ...
    case 'explain_authorization_decision': {
      ...
      const [ruleResponse, combinationsResponse] = await Promise.all([
        httpGet<{ success: boolean; data: { name: string; ruleDefinition: RuleDefinition } }>(
          `/signature-rules/${ruleId}`,
          orgId,
        ),
        httpGet<{ success: boolean; data: Record<string, number>[] }>(
          `/signature-rules/${ruleId}/combinations`,
          orgId,
        ),
      ]);
    ...
    case 'ingest_transaction': {
      ...
      if (args.amountNormalized !== undefined) body.amountNormalized = args.amountNormalized;
      if (args.currencyNormalized !== undefined) body.currencyNormalized = args.currencyNormalized;
      if (args.country !== undefined) body.country = args.country;
      if (args.counterpartyId !== undefined) body.counterpartyId = args.counterpartyId;
    ...
    case 'create_rule_version': {
      ...
      if (args.window !== undefined) body.window = args.window;
    ...
    case 'deactivate_rule_version': {
      ...
      const body: Record<string, unknown> = {};
      if (args.deactivatedAt !== undefined) body.deactivatedAt = args.deactivatedAt;
    ...
    case 'list_alerts': {
      ...
      if (args.idAccount) params.append('idAccount', args.idAccount as string);
    ...
    case 'list_evaluation_results': {
      const params = new URLSearchParams();
      if (args.idTransaction) params.append('idTransaction', args.idTransaction as string);
      if (args.idAccount) params.append('idAccount', args.idAccount as string);
      if (args.decision) params.append('decision', args.decision as string);
      if (args.limit) params.append('limit', String(args.limit));
    ...
    case 'create_signature_request': {
      ...
      if (args.referenceId !== undefined) body.referenceId = args.referenceId;
      if (args.referenceType !== undefined) body.referenceType = args.referenceType;
      if (args.description !== undefined) body.description = args.description;
    ...
    case 'add_signature': {
      ...
      if (args.ipAddress !== undefined) body.ipAddress = args.ipAddress;
...
const allTools: Tool[] = [...apiTools, ...writeApiTools, ...dbTools, ...explanationTools];
