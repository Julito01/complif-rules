 ...
 */
function validateIdentifier(name: string, type: 'table' | 'schema'): void {
  ...
  if (!validPattern.test(name)) {
    throw new Error(
      `Invalid ${type} name: "${name}". Must start with a letter or underscore, contain only alphanumeric characters and underscores, and be at most 63 characters.`,
    );
  }
}
 ...
 */
export async function inspectTables(schema: string = 'public'): Promise<{
  schema: string;
  tables: string[];
  count: number;
}> {
  ...
  const rows = await executeReadOnlyQuery<{ table_name: string }>(
    `
    SELECT table_name
...
export async function countRows(
  ...
  const rows = await executeReadOnlyQuery<{ count: string }>(query);
...
export async function sampleRows(
  ...
  const rows = await executeReadOnlyQuery<Record<string, unknown>>(query);
 ...
 */
export async function inspectDatabaseSchema(schema: string = 'public'): Promise<{
  schema: string;
  tables: Array<{
    name: string;
    columns: Array<{
  ...
  // 1. Tables
  const tables = await executeReadOnlyQuery<{ table_name: string }>(
  ...
  // 3. Indexes
  const indexes = await executeReadOnlyQuery<{
    tablename: string;
    indexname: string;
    indexdef: string;
  }>(
  ...
  // 4. Primary keys
  const primaryKeys = await executeReadOnlyQuery<{
    table_name: string;
    column_name: string;
  }>(
  ...
  // 5. Foreign keys
  const foreignKeys = await executeReadOnlyQuery<{
    table_name: string;
  ...
  const rowEstimates = await executeReadOnlyQuery<{
    relname: string;
    n_live_tup: string;
  }>(
...
export async function queryTransactions(params: {
  ...
  if (!params.organizationId && !params.accountId) {
    throw new Error(
      'At least one filter is required: organizationId or accountId. ' +
  ...
  const tableCheck = await executeReadOnlyQuery<{ exists: boolean }>(
    `SELECT EXISTS (
  ...
  const rows = await executeReadOnlyQuery<Record<string, unknown>>(query, values);
 ...
 */
export async function handleDbToolCall(
  name: string,
