...
// ─────────────────────────────────────────────────────────────────────────────
function resolveFact(facts, path) {
...
// ─────────────────────────────────────────────────────────────────────────────
function explainCondition(condition, facts, aggregationRefs) {
    // ALL combinator
    if (condition.all) {
...
// ─────────────────────────────────────────────────────────────────────────────
function explainRuleEvaluation(rule, transaction) {
    const aggregationRefs = [];
    ...
    return {
        ...
        note: 'This is a pure explanation. No data was read, written, or persisted. ' +
            'Aggregation values must be pre-computed and provided in the transaction facts.',
    };
}
function explainAggregation(input) {
    ...
    const description = aggUpper === 'COUNT'
...
Output: structured explanation of the evaluation tree.`,
        inputSchema: {
            ...
            properties: {
                rule: {
                    type: 'object',
                    description: 'The rule definition JSON (id_rule, name, enabled, priority, conditions, actions)',
                },
                transaction: {
                    type: 'object',
                    description: 'Transaction facts as a flat/nested object. Use dot notation keys matching rule conditions (e.g., {"transaction": {"amount": 15000, "country": "BR"}, "account": {"risk_score": 80}})',
                },
            },
...
Output: description, pseudo-SQL, steps, and performance notes.`,
        inputSchema: {
            ...
            properties: {
                ...
                field: {
                    type: 'string',
                    description: 'The field to aggregate (e.g., "amount", "amount_normalized"). Not needed for COUNT.',
                },
                window: {
                    type: 'object',
...
// Tool Handler
// ─────────────────────────────────────────────────────────────────────────────
export async function handleExplanationToolCall(name, args) {
    try {
        switch (name) {
            ...
            case 'explain_aggregation': {
                const aggregationType = args.aggregationType;
